...
%TODO \section{Motivation}
 ...
 \subsection{Justification} %relocate?
...
The Semantic Web, on the other hand, contains sources of \textit{information} that can be combined whenever the user needs it. To stick with the social media example, a person can create a piece of content, say a picture of a tree, and share it with his friends. One of his friends then can share this piece of content with a group of people that likes nature pictures. However, everyone can access all references someone makes. So if a person from the nature group notices the tree is ill and comments on this, the original poster can see this comment and act accordingly. On the current web the friend who shared the picture with the group has to be contacted directly by the commenter and then has to relay the message manually.
 ...
 Factorie is written in scala, a relatively new language that interfaces with java. See \cite{scala}. This allows for interoperability between factorie and AgreementMaker. Any algorithm that is implemented in factorie or on top of factorie can therefore be used as a matching algorithm in AgreementMaker.
 ...
 \newpage
 \section{Method} %Controleer overgang
 %hier iets opschrijven?
...
The goal of any ontology matcher is to find an alignment between two ontologies that is as similar as possible to the true alignment. Such an alignment consists of node pairs where the two nodes in each pair come from the two different ontologies. Sometimes a label is added to each pair, indicating the relationship the two nodes have. For example, 'part-of', 'similar-to', and such. However, we only consider equality relationships and thus the label can be omitted.
 ...
 The explanations refer back to the example in Section \ref{example}
  ...
  Another way to take into account the influence of neighbouring nodes is to train a multi-sense node embedding model as described in {appropriate section}, and selecting from the different node senses based on which is closest to the context average. %TODO appropriate section
  ...
  %TODO appropriate section
  %TODO maybe ref too\endcomment
 ...
 From the results and interpretation we can conclude that distributed word representations can currently \textbf{not} find relationships between concepts in ontologies better than existing matchers.
 ...
 \subsection{Discussion}
  ...
  The number of edges is known as it has been provided by the designers of the Ontologies\cite{mice_ontology}, so we can compare the number of edges found with the actual number of edges. If these match, the context extraction works.
  \begin{table}
  \begin{tabular}{l|ll}
  & \multicolumn{2}{c}{\#edges}\\
  Algorithm & expected & actual\\
  \hline
  \textbf{Truth}\cite{mice_ontology} & \multicolumn{2}{c}{\textbf{1000000}} \\
  Monodirectional & & \\
  Bidirectional & & \\
  Bidirectional \& edges & & \\  
  \end{tabular}
