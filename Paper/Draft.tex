...
\section{Background}
 ...
 Zijn alle losse technologien er al? Ja. Hoe? RDF en OWL worden al gebruikt. Google heeft al betere search door Knowledge gRaph (hummingbird) plaatje etc...
 Door lack of adoption is het er niet.
...
\end{minipage}
 ...
 %TODO Meer voorbeelden meer plaatjes (uit hun papers). Eerst ga ik uitleggen..
 ...
 %TODO Cite en in discussion of w2v dit kan helpen. Zo niet: weg
 ...
 % Meer McCallum noemen. 80 plaatjes %TODO
...
The goal of any ontology matcher is to find an alignment between two ontologies that is as similar as possible to the true alignment. Such an alignment consists of node pairs where the two nodes in each pair come from the two different ontologies. Sometimes a label is added to each pair, indicating the relationship the two nodes have. For example, 'part-of', 'similar-to', and such. However, we only consider equality relationships and thus the label can be omitted.
 ...
 %TODO Move sommige naar discussions
...
%  \includegraphics[width=0.7\textwidth]{vowlview}
 ...
 \begin{center}
 \textit{
 Can distributed representations of concepts be used to find relationships between concepts in ontologies better than the best existing matcher?
 } % Zelfde als sectie hypothese?
 \end{center}
...
\paragraph{The algorithm uses distributed representations of concepts} %TODO In de uitleg van het algoritme duidelijk maken dat dit zo is.

Yes. They all use distributed representations since they all convert words into vectors:
\begin{description} \item[pre-trained Word2vec] ... 
\item[] ...
\end{description}
\paragraph{The algorithm finds relationships between concepts in ontologies} %TODO In de system check duidelijk maken.
