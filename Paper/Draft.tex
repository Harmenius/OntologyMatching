 ...
 %TODO Definitions
 %TODO Word index
 %TODO Fix hoofdletters (bv ontology, kb)
 ...
 \caption{To find the best movie, the shown movies need to be filtered by the ones you like and the showtimes need to be filtered by the times you are available. All remaining showtimes need to be cross-references with the preferences of your friends.}
 \label{movies}
 \end{figure}
...
\section{Background}
 ...
 \caption{The first iteration of the Web consisted of static content, written by authors and read by readers.}
 \label{web1}
 \end{figure}
 ...
 \caption{In the second iteration of the Web, readers have become users that can alter and add to the content found on the interactive hubs. This also allows users to interact with each other.}
 \label{web2}
 \end{figure}
 ...
 \caption{On the third iteration of the Web, virtual agents are also able to read and write content and interact with humans (through question answering as an example). Virtual agents may also work together.}
 \label{web3}
 \end{figure}
 ...
 \caption{An example of the Semantic Web represented as a graph of entities (in black) and relationships (in red). It contains many different kinds of entities, including countries and people.}
 \label{semantic_web}
 \end{figure}
 ...
 \caption{New information to be added to the Semantic Web.}
 \label{kb}
 \end{figure}
 ...
 \caption{The new information is connected to the rest of the Semantic Web by connecting the new entities with entities in the Semantic Web.}
 \label{combine}
 \end{figure}
 ...
 \caption{The different sub-graphs in the Semantic Web graph. They are contained in different Knowledge Bases and connected by URIs.}
 \label{kbs}
 \end{figure}
 ...
 There are many ways of interacting with the Semantic Web, but they all interact with the graph in some way. For example, a user can ask a question "What food does President Obama like?" Firstly, this question is converted into a format that can be used by a search system. Then, information from the ontologies is added so that the search can be performed more efficiently. Then the pattern is matched in the Semantic Web graph, listing all results. The results are then combined or filtered such that one answer pattern remains, and that pattern is converted to natural text. This process is depicted in Figure \ref{question_graph}.
...
The goal of any ontology matcher is to find an alignment between two ontologies that is as similar as possible to the true alignment. Such an alignment consists of node pairs where the two nodes in each pair come from the two different ontologies. Sometimes a label is added to each pair, indicating the relationship the two nodes have. For example, 'part-of', 'similar-to', and such. However, we only consider equality relationships and thus the label can be omitted.
 ...
 %Further research: grote dataset maken door LOD te pakken, 10% weg te gooien en kijken of je dat reconstruct. 
  ...
  % Uit mijn onderzoek blijkt dat McCallum's algoritme niet werkt op SW. Als ik terugkijk naar zijn keynote, is dat wel logisch, want in zijn voorbeeld heeft hij een dataset gegeven waar de KBs toevallig dezelfde Named Entities hebben (perfect zijn). In de echte wereld is dit met de meeste data niet zo, helemaal in het SW omdat veel ambiguiteit door auatomatscih maken.
  
 % Plaatje van mccallum matrix. Kijk, Steve heet Steven in alle drie de KBs. Dit is in het echt dus niet zo.
